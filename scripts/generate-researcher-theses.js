...
async function run() {
  ...
  try {
    ...
    if (!existingLibrary.researchGroups || !existingLibrary.researchGroups.structured) {
      console.error("\n❌ Research groups data missing. Run the research groups generator to capture structured groups before extracting thesis prompts.");
      return;
    }
    ...
    console.log(`\nMock library updated with researcher theses: ${path.relative(workingDir, DEFAULT_OUTPUT_PATH)}`);
  } catch (error) {
